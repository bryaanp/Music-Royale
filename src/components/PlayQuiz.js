...
export default function PlayQuiz() {
    ...
    //Handles indexing of array which contains Question count (user allowed to choose # of questions)
    const nextQuestion = () => {
        if (questionCounter < questionsArray.length + 1) {
            setQuesCounter(questionCounter + 1)
        }
    }
    //Handles Submission of Quiz which then gives out results
    const submitQuiz = () => {
        addDoc(databaseRef, {
